...
const E = {
  ...
  cook: String.fromCodePoint(0x1F373),
  ...
  picframe: String.fromCodePoint(0x1F5BC),
  ...
  recycle: String.fromCodePoint(0x1F504),
};
...
const ICONS    = {Dashboard:"" + E.house + "",Calendar:"" + E.cal + "",Alarms:"" + E.alarm + "",Notes:"" + E.notes + "",Recipes:"" + E.cook + "",Files:"" + E.folder + "",Locations:"" + E.mappin + "",Bills:"" + E.money + ""};
...
const DEFAULT_DATA = {
  ...
  notes: [
    ...
    {id:2,title:"Self-care reminder",content:"You are doing amazing. Rest is productive. " + E.sparkle + "",color:"#fce4ec",pinned:true,date:"2026-03-01",folder:"Personal",photos:[],links:[]},
  ],
    ...
    const tick = setInterval(() => {
      ...
      alarms.forEach(a => {
        ...
        if (Notification.permission === "granted") {
          new Notification("MomBase " + E.alarm + "", { body: a.label });
        }
      });
    }, 10000);
  ...
  return (
    <div>
      <div style={{marginBottom:28}}>
        <h1 style={{fontFamily:AF,margin:0,color:p.a1,fontSize:36}}>{greeting} {E.flower}</h1>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(145px,1fr))",gap:14,marginBottom:28}}>
        {[
          {l:"Today's Events",v:te.length,i:"' + E.cal + '",bg:`${p.a1}15`},
          {l:"Unpaid Bills",  v:ub.length,i:"" + E.money + "",bg:`${p.a2}15`},
          {l:"Alarms Today",  v:aa.length,i:"" + E.alarm + "",bg:`${p.a3}30`},
          {l:"Pinned Notes",  v:pn.length,i:"" + E.pushpin + "",bg:`${p.gold}20`},
        ].map(s=>(
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(240px,1fr))",gap:16}}>
        <Card>
          <div style={{fontFamily:AF,color:p.a1,marginBottom:12,fontSize:16}}>{E.cal} Today's Schedule</div>
          {te.length ? te.map(e=>(
          ...
          )) : <p style={{color:p.muted,fontSize:13,margin:0,fontStyle:"italic"}}>Free day — enjoy it! {E.party}</p>}
        </Card>

        <Card>
          <div style={{fontFamily:AF,color:p.a1,marginBottom:12,fontSize:16}}>{E.alarm} Alarms Today</div>
          {aa.length ? aa.map(a=>(
        ...
        <Card>
          <div style={{fontFamily:AF,color:p.a1,marginBottom:12,fontSize:16}}>{E.money} Bills to Pay</div>
          {ub.slice(0,4).map(b=>(
          ...
          {!ub.length&&<p style={{color:p.muted,fontSize:13,margin:0,fontStyle:"italic"}}>All bills paid! {E.party}</p>}
        </Card>

        <Card>
          <div style={{fontFamily:AF,color:p.a1,marginBottom:12,fontSize:16}}>{E.pushpin} Pinned Notes</div>
          {pn.slice(0,3).map(n=>(
  ...
  return (
    <div>
      ...
      <div style={{display:"flex",flexWrap:"wrap",gap:10,justifyContent:"space-between",alignItems:"center",marginBottom:20}}>
        <SectionTitle>{E.cal} Calendar</SectionTitle>
      ...
      <Card style={{marginBottom:16,background:`${p.soft}`,border:`1px dashed ${p.a3}`,padding:"12px 16px"}}>
        <div style={{fontSize:12,fontWeight:700,color:p.a1,marginBottom:8,fontFamily:AF}}>{E.link} Connect Your Calendars</div>
        <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
          {[
            {name:"Google Calendar",url:GOOGLE_CAL,color:"#4285F4",icon:"" + E.cal + ""},
            {name:"Outlook",url:OUTLOOK_CAL,color:"#0078D4",icon:"" + E.email + ""},
            {name:"Apple Calendar",url:"https://www.icloud.com/calendar",color:"#555",icon:"" + E.apple + ""},
          ].map(cal=>(
            ...
            <Card key={e.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"12px 16px"}}>
              <div style={{display:"flex",gap:12,alignItems:"center"}}>
                ...
                <div>
                  ...
                  <div style={{fontSize:12,color:p.muted}}>{e.date}{e.time?` · ${e.time}`:""}{e.location?` · ${E.mappin}${e.location}`:""}</div>
            ...
            <div key={e.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"8px 0",borderBottom:`1px solid ${p.border}`}}>
              <div style={{display:"flex",gap:10,alignItems:"center"}}>
                ...
                <div>
                  ...
                  {e.location&&<div style={{fontSize:12,color:p.muted}}>{E.mappin} {e.location}</div>}
                  {e.desc&&<div style={{fontSize:12,color:p.muted,fontStyle:"italic"}}>{e.desc}</div>}
                </div>
              </div>
              <div style={{display:"flex",gap:6}}>
  ...
  return (
    <div>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}>
        <SectionTitle>{E.alarm} Alarms & Reminders</SectionTitle>
      ...
      {Notification.permission!=="granted"&&!permAsked&&(
        <Card style={{marginBottom:16,background:`${p.a3}25`,border:`1px dashed ${p.a3}`,padding:"12px 16px"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <span style={{fontSize:13,color:p.a1,fontFamily:FF}}>{E.bell} Enable notifications so alarms can alert you even when you're not looking at the screen</span>
        ...
        {data.alarms.map(a=>(
          <Card key={a.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",opacity:a.active?1:.55,borderLeft:`4px solid ${a.active?p.a1:p.border}`}}>
            <div>
              ...
              <div style={{fontSize:11,color:p.muted,marginBottom:8}}>{E.bell} {ALARM_SOUNDS.find(s=>s.id===a.sound)?.label||"Gentle Chime"}</div>
            ...
            <div style={{display:"flex",gap:10,alignItems:"center"}}>
              <button onClick={()=>playChime(a.sound||"chime1")} style={{background:"none",border:`1px solid ${p.border}`,borderRadius:8,padding:"4px 10px",cursor:"pointer",fontSize:13,color:p.muted,fontFamily:FF}}>{E.play} Test</button>
              {/* Toggle */}
            ...
            <div style={{display:"flex",flexDirection:"column",gap:6}}>
              {ALARM_SOUNDS.map(s=>(
                <div key={s.id} style={{display:"flex",alignItems:"center",gap:10}}>
                  ...
                  <button onClick={()=>playChime(s.id)} style={{background:"none",border:`1px solid ${p.border}`,borderRadius:6,padding:"3px 8px",cursor:"pointer",fontSize:12,color:p.muted,fontFamily:FF}}>{E.play}</button>
                </div>
              ))}
            </div>
          </div>
      ...
      <div style={{background:note.color,borderRadius:14,padding:16,border:`1px solid ${p.border}`,display:"flex",flexDirection:"column",gap:8}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
          ...
          <div style={{display:"flex",gap:4,flexShrink:0}}>
            {note.folder&&<span style={{fontSize:10,background:"rgba(0,0,0,.08)",borderRadius:10,padding:"2px 6px",fontFamily:FF,color:p.muted}}>{E.folder}{note.folder}</span>}
            <button onClick={()=>update("notes",data.notes.map(n=>n.id===note.id?{...n,pinned:!n.pinned}:n))} style={{background:"none",border:"none",cursor:"pointer",fontSize:14}}>{note.pinned?"" + E.pushpin + "":"" + E.mappin + ""}</button>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:4}}>
            {note.links.map((lk,i)=>(
              <div key={i} style={{display:"flex",alignItems:"center",gap:6}}>
                <a href={lk.startsWith("http")?lk:`https://${lk}`} target="_blank" rel="noreferrer" style={{fontSize:12,color:p.a2,textDecoration:"none",flex:1,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{E.link} {lk}</a>
        ...
        <div style={{display:"flex",gap:6,flexWrap:"wrap",borderTop:`1px solid rgba(0,0,0,.06)`,paddingTop:8}}>
          <button onClick={()=>notePhotoRef.current?.click()} style={{background:"rgba(255,255,255,.6)",border:`1px solid ${p.border}`,borderRadius:6,padding:"3px 8px",cursor:"pointer",fontSize:11,color:p.muted,fontFamily:FF}}>{E.camera} Photo</button>
          <input ref={notePhotoRef} type="file" accept="image/*" style={{display:"none"}} onChange={addNotePhoto}/>
          <button onClick={()=>setAddingLink(a=>!a)} style={{background:"rgba(255,255,255,.6)",border:`1px solid ${p.border}`,borderRadius:6,padding:"3px 8px",cursor:"pointer",fontSize:11,color:p.muted,fontFamily:FF}}>{E.link} Link</button>
        </div>
  ...
  return (
    <div>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
        <SectionTitle>{E.notes} Notes</SectionTitle>
        <div style={{display:"flex",gap:8}}>
          <Btn small variant="soft" onClick={()=>setFolderModal(true)}>{E.folder} Folders</Btn>
          <Btn small onClick={()=>setModal(true)}>+ Note</Btn>
        </div>
      </div>

      <Inp placeholder={E.search + " Search notes…"} value={search} onChange={e=>setSearch(e.target.value)} style={{marginBottom:14}}/>
      ...
      {pinned.length>0&&<>
        <div style={{fontSize:11,fontWeight:700,color:p.muted,marginBottom:8,textTransform:"uppercase",letterSpacing:1,fontFamily:FF}}>{E.pushpin} Pinned</div>
      ...
      <Modal show={modal} onClose={()=>setModal(false)} title="New Note" wide>
        <FormCol>
          ...
          <Sel value={form.folder||""} onChange={e=>setForm(f=>({...f,folder:e.target.value}))}>
            <option value="">{E.folder} No folder</option>
          ...
          <div>
            ...
            {(form.links||[]).map((lk,i)=>(
              <div key={i} style={{display:"flex",alignItems:"center",gap:6,marginBottom:4}}>
                <span style={{fontSize:12,color:p.a2,flex:1,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{E.link} {lk}</span>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:6,marginBottom:8}}>
            {(data.noteFolders||[]).map(f=>(
              <div key={f} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"8px 12px",background:p.soft,borderRadius:8}}>
                <span style={{fontFamily:FF}}>{E.folder} {f}</span>
...
function RecipesSection({data,update}) {
        ...
        <Card>
          ...
          {r.link&&(
            <a href={r.link.startsWith("http")?r.link:`https://${r.link}`} target="_blank" rel="noreferrer"
              ...
              {E.link} View Original Recipe {E.uparrow}
            </a>
          )}
          <h4 style={{color:p.a2,fontFamily:AF,marginBottom:8}}>{E.receipt} Ingredients</h4>
          ...
          <h4 style={{color:p.a2,fontFamily:AF,marginBottom:8}}>{E.woman}{E.cook} Instructions</h4>
  ...
  return (
    <div>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
        <SectionTitle>{E.cook} Recipes</SectionTitle>
        <Btn onClick={()=>setModal(true)}>+ Save Recipe</Btn>
      </div>
      <Inp placeholder={E.search + " Search by name or tag…"} value={search} onChange={e=>setSearch(e.target.value)} style={{marginBottom:20}}/>
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(220px,1fr))",gap:14}}>
        {filtered.map(r=>(
          <Card key={r.id} onClick={()=>setSel(r.id)} style={{cursor:"pointer"}}>
            <div style={{fontSize:40,marginBottom:10}}>{E.plate}</div>
            ...
            {r.link&&<div style={{fontSize:11,color:p.a2,marginBottom:8,fontFamily:FF}}>{E.link} Has source link</div>}
            <div style={{display:"flex",gap:4,flexWrap:"wrap"}}>
              {(r.tags||[]).map(t=><TagBadge key={t}>#{t}</TagBadge>)}
            </div>
          </Card>
      ...
      <Modal show={modal} onClose={()=>setModal(false)} title="Save a Recipe" wide>
        <FormCol>
          ...
          <Inp placeholder={E.link + " Link to online recipe (optional)"} value={form.link} onChange={e=>setForm(f=>({...f,link:e.target.value}))}/>
...
function FilesSection({data,update}) {
  ...
  return (
    <div>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
        <SectionTitle>{E.folder} Files & Photos</SectionTitle>
        ...
        {filtered.map(f=>(
          <Card key={f.id} style={{textAlign:"center",position:"relative",padding:14}} onClick={f.dataUrl?()=>setPreview(f):undefined}>
            ...
            {f.dataUrl&&f.type==="photo" ? (
              ...
              <div style={{fontSize:40,marginBottom:8}}>{E.doc}</div>
            ) : (
              <div style={{fontSize:44,marginBottom:10}}>{f.type==="photo"?"" + E.picframe + "":"" + E.doc + ""}</div>
            )}
            ...
            {f.dataUrl&&(
              <a href={f.dataUrl} download={f.name} onClick={e=>e.stopPropagation()} style={{display:"block",marginTop:6,fontSize:11,color:p.a1,fontWeight:700,textDecoration:"none",fontFamily:FF}}>{E.down} Download</a>
            )}
          </Card>
        ))}
      </div>
        ...
        <FormCol>
          ...
          <div style={{border:`2px dashed ${p.a3}`,borderRadius:12,padding:20,textAlign:"center",cursor:"pointer",background:p.soft}} onClick={()=>fileRef.current?.click()}>
            {form.dataUrl&&form.type==="photo" ? (
              ...
              <div style={{fontSize:13,color:p.a1}}>{E.doc} {form.name} ready to upload</div>
            ) : (
              <>
                <div style={{fontSize:36,marginBottom:8}}>{E.folder}</div>
          ...
          <div style={{display:"flex",gap:8}}>
            {["file","photo"].map(t=>(
              <div key={t} onClick={()=>setForm(f=>({...f,type:t}))} style={{flex:1,padding:10,borderRadius:10,cursor:"pointer",textAlign:"center",fontWeight:700,fontFamily:FF,background:form.type===t?p.a1:p.tag,color:form.type===t?"#fff":p.muted}}>
                {t==="file"?"" + E.doc + " File":"" + E.picframe + " Photo"}
              </div>
            ))}
          </div>
...
function LocationsSection({data,update}) {
  ...
  return (
    <div>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}>
        <SectionTitle>{E.mappin} Saved Locations</SectionTitle>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(240px,1fr))",gap:12}}>
            {data.locations.filter(l=>l.category===cat).map(l=>(
              <Card key={l.id} style={{background:CAT_COLORS[cat]||"#f9f4fb"}}>
                ...
                {l.address&&<div style={{fontSize:13,color:p.muted,marginBottom:3,fontFamily:FF}}>{E.mappin} {l.address}</div>}
                {l.phone&&<div style={{fontSize:13,color:p.muted,marginBottom:3,fontFamily:FF}}>{E.phone} {l.phone}</div>}
                {l.notes&&<div style={{fontSize:13,color:p.text,fontStyle:"italic",marginTop:6,fontFamily:FF}}>"{l.notes}"</div>}
                {l.address&&<a href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(l.address)}`} target="_blank" rel="noreferrer" style={{fontSize:12,color:p.a1,fontWeight:700,textDecoration:"none",display:"block",marginTop:10,fontFamily:FF}}>{E.map} Open in Maps →</a>}
              </Card>
            ))}
          </div>
        </div>
...
function BillsSection({data,update}) {
  ...
  return (
    <div>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
        <SectionTitle>{E.money} Bills & Budget</SectionTitle>
        ...
        {data.bills.sort((a,b)=>a.due-b.due).map(b=>(
          <Card key={b.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"12px 16px",opacity:b.paid?.65:1,borderLeft:`4px solid ${b.paid?p.a2:p.danger}`}}>
            <div>
              ...
              <div style={{fontSize:12,color:p.muted,fontFamily:FF}}>Due day {b.due} · {b.category}{b.auto?" · " + E.recycle + " Auto-pay":""}</div>
            </div>
            <div style={{display:"flex",gap:8,alignItems:"center"}}>
      ...
      <div style={{
        ...
        <div style={{padding:"22px 16px 16px",display:"flex",alignItems:"center",gap:10,borderBottom:`1px solid ${p.sidebarH}`}}>
          {sidebarOpen&&<div style={{flex:1,minWidth:0}}>
            ...
            <div style={{color:`${p.a3}cc`,fontSize:11,marginTop:2,fontFamily:FF}}>your life, organized {E.flower}</div>
          </div>}
          <button onClick={()=>setSidebarOpen(o=>!o)} style={{background:"none",border:"none",color:`${p.a3}cc`,cursor:"pointer",fontSize:18,flexShrink:0,padding:4,lineHeight:1}}>{sidebarOpen?"◀":"" + E.play + ""}</button>
        </div>
        ...
        {sidebarOpen&&<div style={{padding:"12px 16px 20px",color:"#c4856a",fontSize:11,textAlign:"center",borderTop:`1px solid ${p.sidebarH}`,fontFamily:FF}}>
          You're doing great today ' + E.heart + '
        </div>}
      </div>
